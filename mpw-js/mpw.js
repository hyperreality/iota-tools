...
class MPW {
    ...
    // Because HMAC-256 requires 512 bits of input
    // For our purposes, we need 1024
		let key = window.scrypt(password, salt, 32768/*= n*/, 8/*= r*/, 2/*= p*/, 128/*= buflen*/);
	...
	calculateSeed(site, counter = 1, context = null, NS = MPW.NS) {
		...
		if (window.crypto.subtle) {
			return this.key.then(
				// Sign data using HMAC-SHA-384 w/ this.key
			...
			return this.key.then(function (key) {
				...
				// Sign data using HMAC-SHA-384 w/ key
				return CryptoJS.HmacSHA384(data, key)/*= seed*/;
			}).then(function (hash) {
	...
	generate(site, counter = 1, context = null, template = "trytes", NS = MPW.NS) {
      ...
      var i, j, chunked, chunk = 4;
      for (i=0, j=seed.length; i<j; i+=chunk) {
          var slice = seed.slice(i, i+chunk);
          var word = wordsFromBytes(slice);
          words.push(word);
      }
      // console.log(words);

      var trit_arr = words_to_trits(words);
      var tryte_arr = trytes(trit_arr);
	...
	generatePassword(site, counter = 1, template = "trytes") {
